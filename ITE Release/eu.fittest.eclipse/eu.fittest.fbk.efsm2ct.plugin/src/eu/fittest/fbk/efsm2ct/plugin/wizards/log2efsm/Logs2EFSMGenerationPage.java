...
import eu.fittest.fbk.efsm2ct.plugin.wizards.WUtil;

public class Logs2EFSMGenerationPage extends WizardPage {
	...
	protected Logs2EFSMGenerationPage(String pageName, String outputFileName) {
		super(pageName);
	...
	private void dialogChanged() {
		if (!_outputModelFile.getText().endsWith(".efsm")) {
			updateStatus("Output file name must end with '.efsm'");
		} else if (_outputModelFile.getText().contains(File.separator)) {
			updateStatus("No directories can be specified, enter a filen name ending in '.efsm' only");
		} else {
			updateStatus(null);
		}
	}
