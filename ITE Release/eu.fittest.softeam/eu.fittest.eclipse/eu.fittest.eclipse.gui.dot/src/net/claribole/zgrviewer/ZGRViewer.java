 ...
 *
 *   $Id: ZGRViewer.java 4961 2013-05-30 20:39:29Z epietrig $
 */
...
public class ZGRViewer implements ZGRApplication {
	...
	// added by CuND
	static String dotProc;

	public void setDotProc(String dotProc) {
		ZGRViewer.dotProc = dotProc;
		cfgMngr.m_DotPath = new File(dotProc);
	}
	...
	void initGUI(boolean acc, boolean viewOnJPanel) {
		...
		if (viewOnJPanel) {
			_panelView = grMngr.createPanelView(
					grMngr.createZVTMelements(true), 100, 100);
			...
			grMngr.createFrameView(grMngr.createZVTMelements(false),
					acc ? View.OPENGL_VIEW : View.STD_VIEW, jmb);
		}
	...
	JMenuBar initViewMenu(boolean accelerationMode) {
		...
		ActionListener a0 = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				} else if (e.getSource() == showBkI) {
					cfgMngr.showBookmarks();
