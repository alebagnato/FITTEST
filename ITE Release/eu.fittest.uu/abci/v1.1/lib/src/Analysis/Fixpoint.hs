...
module Analysis.Fixpoint ( NodeFun
                         , FixGraph (..)
                         , FixSem
                         , NodeId
                         , NodeVal
...
data NodeFunIn a = NodeFunIn { nfInputs    :: ![EdgeVal a]
                             , nfNodeState :: !(Maybe (NodeVal a))
                             , nfPrevRes   :: !(Maybe (EdgeVal a))
                             , nfSharedIn  :: !(SolveState a)
                             }
data NodeFunOut a = NodeFunOut { nfNewState  :: !(Maybe (NodeVal a))
                               , nfOutput    :: !(Maybe (EdgeVal a))
                               , nfSharedOut :: !(SolveState a)
                               }
...
newtype FixGraph a = FixGraph {getGraph :: (Set (NodeId a, NodeId a))}
...
solve :: (Ord (NodeId a), Show (NodeId a)) =>
         FixGraph a                 ->
         FixSem a                   ->
         Map (NodeId a) (EdgeVal a) ->
         Map (NodeId a) (NodeVal a) ->
         SolveState a               ->
         (Map (NodeId a) (NodeVal a), SolveState a)
...
solve' :: (Ord (NodeId a), Show (NodeId a), SolveState a ~ ()) =>
          FixGraph a                 ->
          FixSem   a                 ->
          Map (NodeId a) (EdgeVal a) ->
          Map (NodeId a) (NodeVal a) ->
