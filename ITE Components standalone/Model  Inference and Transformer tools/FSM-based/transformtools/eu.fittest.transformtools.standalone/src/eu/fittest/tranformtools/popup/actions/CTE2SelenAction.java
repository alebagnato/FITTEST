...
public class CTE2SelenAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		if (selectedFile != null && selectedFile.length > 0) {
			...
			SeleniumGenWizard wizard = new SeleniumGenWizard(file.getLocation().toOSString(), file.getProject());
			WizardDialog dialog = new WizardDialog(shell, wizard);
			if (dialog.open() == WizardDialog.OK){
				
				final String packageName = wizard.getParameterPage().getPackageName();
				final String className = wizard.getParameterPage().getClassName();
				final String targetPage = wizard.getParameterPage().getTargetPage();
				final String cteFile = wizard.getParameterPage().getCteFile();
				final String outputFolder = wizard.getParameterPage().getOutputFolder();
				final String domainInputFile = wizard.getParameterPage().getDomainInputFile();
				final String selenDriver = wizard.getParameterPage().getSeleniumDriver();
				final boolean validTestOnly = wizard.getParameterPage().isOnlyValidTestCase();
				...
				Job generateJob = new Job("Generate Test Case"){
					...
					protected IStatus run(final IProgressMonitor monitor) {
						...
						if (selenDriver.equals(Constants.FLEX_OBJECT_DRIVER)){
							templateGroupFile = Activator.getPluginPath().toOSString() + File.separator + "templates" + File.separator + "junit.flexdriver.stg";
						} else if (selenDriver.equals(Constants.FLASH_OBJECT_DRIVER)){
							templateGroupFile = Activator.getPluginPath().toOSString() + File.separator + "templates" + File.separator + "junit.flashdriver.stg";
						} else {
							templateGroupFile = Activator.getPluginPath().toOSString() + File.separator + "templates" + File.separator + "junit4.wdriver.stg";
						}
						...
						if (templateProvider.isTemplateReady()){
							...
							try {
								...
								if (selenDriver.equals(Constants.FLEX_OBJECT_DRIVER)){
									transformer = new CTE2FlexSelenium(templateProvider, packageName, 
											className, targetPage,  selenDriver);
								} else if (selenDriver.equals(Constants.FLASH_OBJECT_DRIVER)){ 
									transformer = new CTE2FlashSelenium(templateProvider, packageName, 
											className, targetPage,  selenDriver);
								} else {
									transformer = new CTE2Selenium(templateProvider, packageName, 
											className, targetPage,  selenDriver);
								}
