...
public class Utility {
	...
	public boolean equalContent(String[] object1, String[] object2) {
		...
		/*
		 * if ((object1.length==0)&&(object2.length==0)) { return true;} else if
		 * ((object1.length==0)||(object2.length==0)) { return false;}
		 */

		boolean iscontaned = true;

		for (int i = 0; i < object2.length; i++) {
			if (!iscontained(object1, object2[i]))
				iscontaned = false;
		}
		if (iscontaned == false) {
			return false;
		...
		for (int i = 0; i < object1.length; i++) {
			if (!iscontained(object2, object1[i]))
				iscontaned = false;
		}
		if (iscontaned == false) {
			return false;
	...
	public boolean iscontained(String[] object1, String str) {
		int i = 0;
		while (i < object1.length) {
			if (object1[i].trim().equals(str.trim()))
				return true;
			i++;
		}
		return false;
	...
	public BufferedWriter createFile(String dir, String fileName, boolean append) {
		try {
			BufferedWriter outF = new BufferedWriter(new FileWriter(dir
					+ System.getProperty("file.separator") + fileName, append));
			outF.flush();
			return outF;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void createFile(String dir, String fileName) {
		try {
			outF = new BufferedWriter(new FileWriter(dir
					+ System.getProperty("file.separator") + fileName));
			outF.flush();
		} catch (Exception e) {
			try {
	...
	public int randomInt(int max, int toskip) {
		// java.util.Random rand = new java.util.Random(19580427);
		int value = rand.nextInt(max);
		while (value == toskip) {
			value = rand.nextInt(max);
		}
	...
	public Vector<File[]> getItems(Vector<File[]> listOfFilesSequences,
	...
	public File[] getArraySubset(File[] tobesplitted, int startindex,
			int stopindex) {
	...
	public void moveFilesInSubdir(String dirSource, String dirTarget,
			int amountOfFilesToMove_Perc) {
		File folderSource = new File(dirSource);
		File[] listOfFiles = folderSource.listFiles();

		createDirectory(folderSource + System.getProperty("file.separator")
				+ dirTarget);
		BufferedWriter wrb;
		BufferedReader inb;
		String fname = "";
		...
		if (amountOfFilesToMove_Perc == -1)
			amountOfFilesToMove_Perc = 30;
					...
					if ((fname.startsWith("log_s")) && (fname.endsWith(".txt"))) {
						wrb = createFile(
								folderSource
										...
										+ dirTarget, fname, false);
						inb = new BufferedReader(new FileReader(folderSource
