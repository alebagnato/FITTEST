...
/**
 * Mutate an input node of type integer or double based on its 
 * upper bound constraint.
 * 
 * @author cdnguyen
 *
 */
public class MaxInclusiveOperator implements IMutationOperator {
	...
	public Node mutate(Node input, OnElementType constraint) {
		...
		for (Object o : facets) {
			...
			if (o instanceof JAXBElement) {
				...
				if (constraintType.equals(IMutationOperator.XSD_MUTATION_MAXINCLUSIVE)) {
					...
					if (min != null){
						try {
							...
							if (firstChild != null){
								firstChild.setTextContent(String.valueOf((int)(min + 1)));
							}
							return mutatedNode;
