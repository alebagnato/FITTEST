...
public class PostOptimizer {
	...
	private static final int  M_SIZE = 1000; // by urueda: avoid ArrayIndexOutOfBoundsException (exact value is displayed to console during optimization)
...
//				return false;
	...
	public void synthesizeInfo(String cteFolder){
		...
		System.out.println("M_SIZE should be: " + globalClzIdentifiers.size()); // by urueda
	}
	...
	private List<TestInfo> postOptimizing() {
		...
		while (!done){
			...
			for (Cell cell : allCells){
				if (cell.unsorted) {
					System.out.println("Sorting: " + cell.tcList.size() + " elements");
				...
				for (TestInfo test : cell.tcList){
					if (test.isRemovable()){
						if (!retList.contains(test)) { // by urueda (has already been removed?)
							done = false;
							retList.add(test);
							toBeRemoved = test;
							System.out.println("Removed (" + retList.size() + " total): " + test.getPrettyId());
						}
						break;
					}
				}
				...
				if (toBeRemoved != null){
					...
					remove(toBeRemoved);
					//break; // commented by urueda (let all cells be optimized to speedup the process)
				}
			}
		}
		return retList;
			...
			for (int j = 0; j < size; j++){
				...
				if (c != null){
					if (c.tcList.contains(toBeRemoved)){
						...
						c.unsorted = true; // by urueda
					}
				}
			}
		}
		// begin by urueda (invalidate cache)
		toBeRemoved.cachedCvg = -1;
		toBeRemoved.cachedRmv = false;
		// end by urueda
	}
...
//		
	...
	private class TestInfo implements Comparable<TestInfo>{
		public int cachedCvg = -1; // by urueda (does not matter the coverage of a removed test case due to optimization)
		public boolean cachedRmv = false; // by urueda (once a test case is not removable, it will not be in further optimization)
		...
		public int getMyCoverage(){
			...
			cachedCvg = counter; // by urueda
			return counter;
		}
		
		public boolean isRemovable(){
			if (cachedRmv) return false; // by urueda
			int size = globalClzIdentifiers.size();
			for (int i = 0; i < size; i++){
				for (int j = 0; j < size; j++){
					Cell c = matrix[i][j];
					if (c != null){
						//if (c.tcList.contains(this)){
						//	if (c.tcList.size() == 1){
						// by urueda: change the order of conditions
						if (c.tcList.size() == 1 && c.tcList.contains(this)){
							cachedRmv = true; // by urueda
							return false;
						}
					}
				}
	...
	private class Cell implements Comparable<Cell>{
		...
		public boolean unsorted = true; // by urueda
