...
public class GraphicsManager implements ComponentListener, CameraListener,
		Java2DPainter {
	...
	VirtualSpace rSpace; // virtual space containing rectangle representing
							// region seen through main camera (used in
							// overview)
	...
	Vector createZVTMelements(boolean applet) {
		...
		// camera for rectangle representing region seen in main viewport (in
		// overview)
		rSpace.addCamera();
		...
		SIRectangle seg1;
		SIRectangle seg2;
		observedRegion = new VRectangle(0, 0, 0, 10, 10,
				ConfigManager.OBSERVED_REGION_COLOR,
				ConfigManager.OBSERVED_REGION_CROSSHAIR_COLOR, 0.5f);
		// 500 should be sufficient as the radar window is
		seg1 = new SIRectangle(0, 0, 0, 0, 500,
				ConfigManager.OBSERVED_REGION_CROSSHAIR_COLOR);
		// not resizable and is 300x200 (see rdW,rdH below)
		seg2 = new SIRectangle(0, 0, 0, 500, 0,
				ConfigManager.OBSERVED_REGION_CROSSHAIR_COLOR);
	...
	void createFrameView(Vector cameras, String vt, JMenuBar jmb) {
		mainView = vsm.addFrameView(cameras, ConfigManager.MAIN_TITLE, vt,
				ConfigManager.mainViewW, ConfigManager.mainViewH, true, false,
				true, jmb);
	...
	void parameterizeView(BaseEventHandler eh) {
		...
		// XXX: TBD: refactor event handler code taking advantage of new one
		// handler per layer functionality
	...
	public void closeView() {
		((JFrame) mainView.getFrame()).dispose();
	}
	...
	void seekBoundingBox() {
		...
		for (int i = 0; i < v.size(); i++) {
			if (v.elementAt(i) instanceof VRectangleOr) {
				...
				if (largestRectangle == null || bigger(r, largestRectangle)) {
					// first rectangle encountered in the list, or compare this
					// rectangle to biggest rectangle at this time
					largestRectangle = r;
					lri = i;
				}
			}
		...
		// XXX: disabled check for objects lower in the stack as their seems to
		// be some unidentified object that messes around
		// in some versions of GraphViz such as 2.14 - this workaround should be
		// fairly safe until we identify that object
		// for (int i=0;i<lri;i++){
		...
		// If they are, then it is very likely that the rectangle is a bounding
		// box.
	...
	boolean containedIn(Glyph g, VRectangle r) {
		if (g instanceof DPath || g instanceof VText) {
			return true;// don't take edges and text into accout, would be too
						// costly (and would require one repaint for text)
		} else {// just get geometrical center for other glyphs ; this is an
				// approximation, but it should work
			return g.vx > r.vx - r.getWidth() && g.vx < r.vx + r.getWidth()
					&& g.vy > r.vy - r.getHeight()
					&& g.vy < r.vy + r.getHeight();
		}
	}
	...
	public void getGlobalView() {
		Location l = mainView.getGlobalView(mSpace.getCamera(0),
	...
	public void getHigherView() {
		...
		Animation a = animator.getAnimationFactory().createCameraAltAnim(
				ConfigManager.ANIM_MOVE_LENGTH, c, alt, true,
	...
	public void translateView(short direction) {
		...
		Animation a = animator.getAnimationFactory().createCameraTranslation(
				ConfigManager.ANIM_MOVE_LENGTH, c, trans, true,
				SlowInSlowOutInterpolator.getInstance(), null);
		animator.startAnimation(a, false);
	}
	...
	public void goTo(Location l) {
		Vector animParams = Location.getDifference(mSpace.getCamera(0)
				.getLocation(), l);
		Animation at = animator.getAnimationFactory().createCameraTranslation(
				ConfigManager.ANIM_MOVE_LENGTH, mSpace.getCamera(0),
				(Point2D.Double) animParams.elementAt(1), true,
				SlowInSlowOutInterpolator.getInstance(), null);
		Animation aa = animator.getAnimationFactory().createCameraAltAnim(
				ConfigManager.ANIM_MOVE_LENGTH, mSpace.getCamera(0),
				(Double) animParams.elementAt(0), true,
				SlowInSlowOutInterpolator.getInstance(), null);
	...
	public void showRadarView(boolean b) {
		if (b) {
			if (rView == null) {
				...
				vsm.addFrameView(cameras, RADAR_VIEW_NAME, View.STD_VIEW,
						ConfigManager.rdW, ConfigManager.rdH, true);
				...
				// same event handler handling all layers for now
				// XXX: TBD: refactor event handler code taking advantage of new
				// one-handler-per-layer functionality
				...
				mSpace.getCamera(1).setLocation(
						rView.getGlobalView(mSpace.getCamera(1)));
	...
	public void cameraMoved(Camera cam, Point2D.Double coord, double alt) {
		if (rView != null) {
			...
			observedRegion.moveTo((wnes[0] + wnes[2]) / 2,
					(wnes[3] + wnes[1]) / 2);
	...
	public void zoomInPhase1(int x, int y) {
		...
		Animation a = animator.getAnimationFactory().createLensMagAnim(
				LENS_ANIM_TIME, (FixedSizeLens) lens,
				new Float(MAG_FACTOR - 1), true,
	...
	public void zoomInPhase2(double mx, double my) {
		...
		Double deltAlt = new Double((cameraAbsAlt) * (1 - MAG_FACTOR)
				/ MAG_FACTOR);
		if (cameraAbsAlt + deltAlt.floatValue() > FLOOR_ALTITUDE) {
			Animation al = animator.getAnimationFactory()
					.createLensMagAnim(LENS_ANIM_TIME, (FixedSizeLens) lens,
							new Float(-MAG_FACTOR + 1), true,
							IdentityInterpolator.getInstance(),
							new ZP2LensAction(this));
			Animation at = animator.getAnimationFactory()
					.createCameraTranslation(
							LENS_ANIM_TIME,
							mainCamera,
							new Point2D.Double(c2x - mainCamera.vx, c2y
									- mainCamera.vy), true,
							IdentityInterpolator.getInstance(), null);
			Animation aa = animator.getAnimationFactory().createCameraAltAnim(
					LENS_ANIM_TIME, mainCamera, deltAlt, true,
					IdentityInterpolator.getInstance(), null);
			...
			Animation at = animator.getAnimationFactory()
					.createCameraTranslation(
							LENS_ANIM_TIME,
							mainCamera,
							new Point2D.Double((c2x - mainCamera.vx) * ratio,
									(c2y - mainCamera.vy) * ratio), true,
							IdentityInterpolator.getInstance(), null);
			Animation aa = animator.getAnimationFactory().createCameraAltAnim(
					LENS_ANIM_TIME, mainCamera, actualDeltAlt, true,
					IdentityInterpolator.getInstance(), null);
			animator.startAnimation(al, false);
			animator.startAnimation(at, false);
			animator.startAnimation(aa, false);
	...
	public void zoomOutPhase1(int x, int y, double mx, double my) {
		...
		Animation al = animator.getAnimationFactory().createLensMagAnim(
				LENS_ANIM_TIME, (FixedSizeLens) lens,
				new Float(MAG_FACTOR - 1), true,
				IdentityInterpolator.getInstance(), null);
		Animation at = animator.getAnimationFactory().createCameraTranslation(
				LENS_ANIM_TIME, mainCamera,
				new Point2D.Double(c2x - mainCamera.vx, c2y - mainCamera.vy),
				true, IdentityInterpolator.getInstance(), null);
		Animation aa = animator.getAnimationFactory().createCameraAltAnim(
				LENS_ANIM_TIME, mainCamera,
				new Double(cameraAbsAlt * (MAG_FACTOR - 1)), true,
				IdentityInterpolator.getInstance(), null);
		animator.startAnimation(al, false);
		animator.startAnimation(at, false);
		animator.startAnimation(aa, false);
	...
	public void zoomOutPhase2() {
		...
		Animation a = animator.getAnimationFactory().createLensMagAnim(
				LENS_ANIM_TIME, (FixedSizeLens) lens,
				new Float(-MAG_FACTOR + 1), true,
				IdentityInterpolator.getInstance(), new ZP2LensAction(this));
		animator.startAnimation(a, false);
	}
	...
	public synchronized void magnifyFocus(double magOffset, int zooming,
			Camera ca) {
		synchronized (lens) {
			...
			if (nmf <= MAX_MAG_FACTOR && nmf > 1.0f) {
				...
				if (zooming == GraphicsManager.ZOOMOUT_LENS) {
					 ...
					 * - a1)
					 */
					mainCamera.altitudeOffset((a1 + mainCamera.getFocal())
							* magOffset / (MAG_FACTOR - magOffset));
					/*
					 ...
					 */
					mainCamera.move(
							(a1 - mainCamera.getAltitude()
									/ mainCamera.getFocal() * lens.lx),
							-(a1 - mainCamera.getAltitude()
									/ mainCamera.getFocal() * lens.ly));
				} else {
					Animation a = animator.getAnimationFactory()
							.createLensMagAnim(WHEEL_ANIM_TIME,
									(FixedSizeLens) lens, new Float(magOffset),
	...
	Lens getLensDefinition(int x, int y) {
		...
		if (tp.isFadingLensNavMode()) {
			fLens = new LInfSCBLens(1.0f, 0.0f, 0.95f, 100, x - panelWidth / 2,
					y - panelHeight / 2);
			res = fLens;
		} else {
			// isProbingLensNavMode()
			res = new FSGaussianLens(1.0f, 100, 50, x - panelWidth / 2, y
					- panelHeight / 2);
			// unset any previous fading lens to make sure it gets garbage
			// collected
			fLens = null;
		}
		return res;
	...
	void createDM(int x, int y, PortalListener pl) {
		dmPortal = new DraggableCameraPortal(x, y,
				GraphicsManager.DM_PORTAL_WIDTH,
				GraphicsManager.DM_PORTAL_HEIGHT, dmCamera);
		...
		dmCamera.setAltitude(((mainCamera.getAltitude() + mainCamera.getFocal())
				/ (DEFAULT_MAG_FACTOR) - mainCamera.getFocal()));
		updateMagWindow();
		int w = (int) Math.round(magWindow.getWidth() * mainCamera.getFocal()
				/ ((float) (mainCamera.getFocal() + mainCamera.getAltitude())));
		int h = (int) Math.round(magWindow.getHeight() * mainCamera.getFocal()
				/ ((float) (mainCamera.getFocal() + mainCamera.getAltitude())));
		dmPortal.sizeTo(w, h);
		mSpace.onTop(magWindow);
		mSpace.show(magWindow);
		paintLinks = true;
		// animator.createPortalAnimation(GraphicsManager.DM_PORTAL_ANIM_TIME,
		// AnimManager.PT_SZ_TRANS_LIN, data, dmPortal.getID(), null);
		Animation as = animator.getAnimationFactory().createPortalSizeAnim(
				GraphicsManager.DM_PORTAL_ANIM_TIME, dmPortal,
				GraphicsManager.DM_PORTAL_WIDTH - w,
				GraphicsManager.DM_PORTAL_HEIGHT - h, true,
				IdentityInterpolator.getInstance(), null);
		Animation at = animator.getAnimationFactory().createPortalTranslation(
				GraphicsManager.DM_PORTAL_ANIM_TIME,
				dmPortal,
				new Point(GraphicsManager.DM_PORTAL_INITIAL_X_OFFSET - w / 2,
						GraphicsManager.DM_PORTAL_INITIAL_Y_OFFSET - h / 2),
				true, IdentityInterpolator.getInstance(), null);
	...
	public void paint(Graphics2D g2d, int viewWidth, int viewHeight) {
		if (paintLinks) {
			double coef = mainCamera.focal
					/ (mainCamera.focal + mainCamera.altitude);
			int magWindowX = (int) Math.round((viewWidth / 2)
					+ (magWindow.vx - mainCamera.vx) * coef);
			int magWindowY = (int) Math.round((viewHeight / 2)
					- (magWindow.vy - mainCamera.vy) * coef);
			...
			g2d.drawLine(magWindowX - magWindowW, magWindowY - magWindowH,
					dmPortal.x, dmPortal.y);
			g2d.drawLine(magWindowX + magWindowW, magWindowY - magWindowH,
					dmPortal.x + dmPortal.w, dmPortal.y);
			g2d.drawLine(magWindowX - magWindowW, magWindowY + magWindowH,
					dmPortal.x, dmPortal.y + dmPortal.h);
			g2d.drawLine(magWindowX + magWindowW, magWindowY + magWindowH,
					dmPortal.x + dmPortal.w, dmPortal.y + dmPortal.h);
		}
	...
	void search(String s, int direction) {
		if (s.length() > 0) {
			if (!s.toLowerCase().equals(lastSearchedString)) {// searching a new
																// string -
																// reinitialize
																// everything
				resetSearch(s);
				...
				for (int i = 0; i < gl.length; i++) {
					if (gl[i] instanceof VText) {
						if ((((VText) gl[i]).getText() != null)
								&& (((VText) gl[i]).getText().toLowerCase()
										.indexOf(lastSearchedString) != -1)) {
			...
			if (matchSize > 0) {
				...
				if (searchIndex < 0) {// if reached start/end of list, go to
										// end/start (loop)
				...
				if (matchSize > 1) {
					zapp.setStatusBarText(AppletUtils
							.rankString(searchIndex + 1)
							+ " of "
							+ matchSize
							+ " matches");
				} else {
				...
				mainView.centerOnGlyph(g, mSpace.getCamera(0),
						ConfigManager.ANIM_MOVE_LENGTH, true,
						ConfigManager.MAG_FACTOR * 1.5f);
				highlight(g);
				vsm.repaint();
			} else {
	...
	void assignFontToGraph() {
		Font f = fr.inria.zvtm.widgets.FontDialog.getFontDialog(
				(JFrame) mainView.getFrame(), ConfigManager.defaultFont);
		if (f != null) {
	...
	public void componentResized(ComponentEvent e) {
		if (e.getSource() == mainView.getFrame()) {
			updatePanelSize();
			// update rectangle showing observed region in radar view when main
			// view's aspect ratio changes
			cameraMoved(null, null, 0);
			// update SD_ZOOM_THRESHOLD
			Dimension sz = mainView.getFrame().getSize();
			cfgMngr.setSDZoomThreshold(0.3 * Math.sqrt(sz.width * sz.width
					+ sz.height * sz.height));
	...
	void buildLogicalStructure() {
		// clone the structure as we are about to remove elements from it for
		// convenience (it is supposed to be read-only)
		 ...
		 */
		glyphs = mSpace.getAllGlyphs(); // not cloning here because we are just
	...
	public LEdge addEdge(LNode sn, LNode en, String title, boolean directed) {
		...
		// find endpoints for the edge by computing intersections between
		// segment
		...
		Area edgeseg = new Area(st.createStrokedShape(new Line2D.Double(sns.vx,
				sns.vy, ens.vx, ens.vy)));
		...
		Point2D.Double cpqd1 = new Point2D.Double(
				(3 * spc.getCenterX() + epc.getCenterX()) / 4d,
				(3 * spc.getCenterY() + epc.getCenterY()) / 4d);
		// point of junction between both quadratic curves
		Point2D.Double jp = new Point2D.Double(
				(spc.getCenterX() + epc.getCenterX()) / 2d,
				(spc.getCenterY() + epc.getCenterY()) / 2d);
		// control point of 2nd quadratic curve
		Point2D.Double cpqd2 = new Point2D.Double(
				(spc.getCenterX() + 3 * epc.getCenterX()) / 4d,
				(spc.getCenterY() + 3 * epc.getCenterY()) / 4d);
		// instantiate all glyphs that form the edge
		final DPath spline = new DPath(spc.getCenterX(), spc.getCenterY(), 0,
				Color.BLACK);
		spline.addQdCurve(jp.x, jp.y, cpqd1.x, cpqd1.y, true);
		spline.addQdCurve(epc.getCenterX(), epc.getCenterY(), cpqd2.x, cpqd2.y,
				true);
		double theta = Math.atan2(epc.getCenterY() - spc.getCenterY(),
				epc.getCenterX() - spc.getCenterX());
		final VShape arrowHead = new VShape(epc.getCenterX(), epc.getCenterY(),
				0, ens.getSize() / 10.0, TRIANGLE_VERTICES, Color.BLACK, theta);
	...
	int highlightLevel;
	// currently selected direction of highlighting neighboring
	// -1/UNDIRECTED/INCOMING/OUTGOING
	int highlightDir;
	...
	// direction of highlighting for the fixated node at according pos of
	// highlightNodes
	Vector highlightDirections = new Vector();

	public void highlightElement(Glyph g, Camera cam, VCursor cursor,
			boolean highlight, int moreLess, boolean fixNode, int dir) {
		Object o = null;
		if (g != null && g != boundingBox) {
			...
			if (otherGlyphs != null && otherGlyphs.size() > 0) {
				...
				// could not detect anything, consider that user clicked on
				// background
		...
		if (o != null) {
			if (fixNode) {
				int indexNode = highlightNodes.indexOf(o);
				if (indexNode >= 0) {
	...
	synchronized void highlightNodeInOut(LNode n, boolean highlight,
			int highlightLevel, int dir) {
		...
		if (dir == LEdge.UNDIRECTED) {
			...
			if (dir != LEdge.OUTGOING)
				highlightNode(n, highlight, LEdge.INCOMING, highlightLevel);
			highlightNodesDone.clear();
			highlightTimeStamp = Calendar.getInstance();
			highlightTimeStamp.add(Calendar.SECOND, highlightTimeStampTimeout);
	...
	synchronized void highlightNode(LNode n, boolean highlight, short dir,
			int levelCnt) {
		if (highlight) {
			...
			for (int i = 0; i < n.edges.length; i++) {
				...
				if (dir != LEdge.UNDIRECTED && dir != n.edgeDirections[i]
						&& n.edges[i].isDirected()) {
					continue;
				}
				...
				highlightNode(n.edges[i].getOtherEnd(n), highlight, dir,
						levelCnt - 1);
			}
		} else {
			unhighlightAll();
	...
	void highlightNodeGlyph(Glyph g) {
		...
		if (g instanceof ClosedShape) {
			// keep both originalXXXColor vectors at the same length/indexes for
			// a given glyph
			originalNodeFillColor.add(null);
			originalNodeBorderColor.add(((ClosedShape) g)
	...
	void highlightEdgeGlyph(Glyph g) {
		...
		if (g instanceof ClosedShape) {
			originalEdgeBorderColor.add(((ClosedShape) g)
					.getDefaultBorderColor());
			if (g.isFilled()) {
	...
	void unhighlightAllNodes() {
		...
		for (int i = 0; i < highlightedNodes.size(); i++) {
			...
			if (g instanceof ClosedShape) {
				if (g.isBorderDrawn()) {
					g.setBorderColor((Color) originalNodeBorderColor
							.elementAt(i));
				}
			} else {
	...
	void unhighlightAllEdges() {
		...
		for (int i = 0; i < highlightedEdges.size(); i++) {
			...
			if (g instanceof ClosedShape) {
				...
				if (g.isBorderDrawn()) {
					g.setBorderColor((Color) originalEdgeBorderColor
							.elementAt(i));
				}
			} else {
				g.setColor((Color) originalEdgeColor.elementAt(i));
	...
	void startBringAndGo(Glyph g) {
		...
		for (int i = 0; i < arcs.length; i++) {
			...
			double d = Math.sqrt((otherEndShape.vx - thisEndShape.vx)
					* (otherEndShape.vx - thisEndShape.vx)
					+ (otherEndShape.vy - thisEndShape.vy)
					* (otherEndShape.vy - thisEndShape.vy));
			// use this end's shape size when getting ring because we're
			// rescaling
			// brought nodes to match the size of the current node
			Ring ring = rm.getRing(Math.atan2(otherEndShape.vy
					- thisEndShape.vy, otherEndShape.vx - thisEndShape.vx),
					thisEndShape.getSize(), RING_STEP);
			double bd = ring.rank * RING_STEP;
			double ratio = bd / d;
			double bx = thisEndShape.vx + ratio
					* (otherEndShape.vx - thisEndShape.vx);
			double by = thisEndShape.vy + ratio
					* (otherEndShape.vy - thisEndShape.vy);
		...
		for (int i = 0; i < arcs.length; i++) {
			...
			LNode otherEnd = arcs[i].getOtherEnd(n);
			ClosedShape otherEndShape = otherEnd.getShape();
			bring(arcs[i], otherEnd, thisEndShape.vx, thisEndShape.vy,
					otherEndShape.vx, otherEndShape.vy, thisEndShape.getSize(),
					node2broughtPosition);
		}
		...
		for (int i = 0; i < elementsToFade.size(); i++) {
			try {
				...
				// XXX:TBW animate only if total number of objects is not too
				// high
				Animation a = animator.getAnimationFactory()
						.createTranslucencyAnim(BRING_ANIM_DURATION, t,
								FADED_ELEMENTS_TRANSLUCENCY, false,
								IdentityInterpolator.getInstance(), null);
	...
	void endBringAndGo(Glyph g) {
		...
		if (!broughtElements.isEmpty()) {
			for (int i = broughtElements.size() - 1; i >= 0; i--) {
				Point2D.Double l = sendBack(
						(BroughtElement) broughtElements.elementAt(i), g);
				if (l != null) {
					newCameraLocation = l;
				}
			}
		...
		for (int i = 0; i < elementsToFade.size(); i++) {
			try {
				...
				// high
				Animation a = animator.getAnimationFactory()
						.createTranslucencyAnim(BRING_ANIM_DURATION,
								(Translucent) elementsToFade.elementAt(i),
								alphaOfElementsToFade[i], false,
								IdentityInterpolator.getInstance(), null);
				animator.startAnimation(a, true);
			} catch (ClassCastException e) {
				e.printStackTrace();
		...
		if (newCameraLocation != null) {
			Animation a = animator.getAnimationFactory()
					.createCameraTranslation(BRING_ANIM_DURATION, mainCamera,
							newCameraLocation, false,
							SlowInSlowOutInterpolator.getInstance(), null);
			animator.startAnimation(a, true);
		}
	}

	void bring(LEdge arc, LNode node, double sx, double sy, double ex,
			double ey, double size, Hashtable node2broughtPosition) {
		...
		if (size < nodeShape.getSize()) {
			Animation szAnim = animator.getAnimationFactory()
					.createGlyphSizeAnim(BRING_ANIM_DURATION, nodeShape, size,
							false, SlowInSlowOutInterpolator.getInstance(),
							null);
		...
		Point2D.Double bposition = (Point2D.Double) node2broughtPosition
				.get(node);
		Point2D.Double translation = new Point2D.Double(bposition.x
				- nodeShape.vx, bposition.y - nodeShape.vy);
		Glyph[] glyphs = node.getGlyphs();
		for (int i = 0; i < glyphs.length; i++) {
			elementsToFade.remove(glyphs[i]);
			Animation posAnim = animator.getAnimationFactory()
					.createGlyphTranslation(BRING_ANIM_DURATION, glyphs[i],
							translation, true,
							SlowInSlowOutInterpolator.getInstance(), null);
		...
		if (Math.sqrt((asp.x - ex) * (asp.x - ex) + (asp.y - ey) * (asp.y - ey)) < Math
				.sqrt((asp.x - sx) * (asp.x - sx) + (asp.y - sy) * (asp.y - sy))) {
		...
		Point2D.Double[] flatCoords = DPath.getFlattenedCoordinates(spline, sp,
				ep, true);
		Animation a = animator.getAnimationFactory().createPathAnim(
				BRING_ANIM_DURATION, spline, flatCoords, false,
				SlowInSlowOutInterpolator.getInstance(), null);
		animator.startAnimation(a, true);
		...
		for (int i = 0; i < glyphs.length; i++) {
			if (glyphs[i] != spline) {
				...
				if (glyphs[i] instanceof VText) {
					// put any label at the center of the edge (simplest thing
					// we can do)
					a = animator.getAnimationFactory().createGlyphTranslation(
							BRING_ANIM_DURATION,
							glyphs[i],
							new Point2D.Double(bposition.x - sx, bposition.y
									- sy), true,
							SlowInSlowOutInterpolator.getInstance(), null);
					animator.startAnimation(a, true);
				} else {
					// probably a tail or head decoration ; just hide it for
					// now, we don't know how to transform them correctly
		...
		for (int i = 0; i < otherArcs.length; i++) {
			broughtElements.add(BroughtElement
					.rememberPreviousState(otherArcs[i]));
			...
			if (node2broughtPosition.containsKey(otherArcs[i].getTail())
				...
				sp = (Point2D.Double) node2broughtPosition.get(otherArcs[i]
						.getTail());
				ep = (Point2D.Double) node2broughtPosition.get(otherArcs[i]
						.getHead());
			} else {
				oe = otherArcs[i].getOtherEnd(node).getShape();
				if (Math.sqrt((asp.x - ex) * (asp.x - ex) + (asp.y - ey)
						* (asp.y - ey)) <= Math.sqrt((aep.x - ex)
						* (aep.x - ex) + (aep.y - ey) * (aep.y - ey))) {
					sp = new Point2D.Double(bposition.x, bposition.y);
					ep = oe.getLocation();
				} else {
					sp = oe.getLocation();
			...
			mSpace.above(spline, boundingBox);
			a = animator.getAnimationFactory().createPathAnim(
					BRING_ANIM_DURATION, spline, flatCoords, false,
					SlowInSlowOutInterpolator.getInstance(), null);
			animator.startAnimation(a, true);
	...
	public void attemptLinkSliding(double press_vx, double press_vy, int scr_x,
			int scr_y) {
		...
		if (lstruct != null) {
			...
			double shortestDistance = Math.sqrt((nodeShape.vx - press_vx)
					* (nodeShape.vx - press_vx) + (nodeShape.vy - press_vy)
					* (nodeShape.vy - press_vy));
			double distance;
			for (int i = 1; i < lstruct.nodes.length; i++) {
				nodeShape = lstruct.nodes[i].getShape();
				distance = Math.sqrt((nodeShape.vx - press_vx)
						* (nodeShape.vx - press_vx) + (nodeShape.vy - press_vy)
						* (nodeShape.vy - press_vy));
			...
			if (shortestDistance < 2 * closestNode.getShape().getSize()) {
				// if clicked near a node, select edge connected to this node
				...
				shortestDistance = Math.sqrt((cPos.getX() - mPos.getX())
						* (cPos.getX() - mPos.getX())
						+ (cPos.getY() - mPos.getY())
						* (cPos.getY() - mPos.getY()));
				for (int i = 1; i < arcs.length; i++) {
					...
					distance = Math.sqrt((cPos.getX() - mPos.getX())
							* (cPos.getX() - mPos.getX())
							+ (cPos.getY() - mPos.getY())
							* (cPos.getY() - mPos.getY()));
					if (distance < shortestDistance) {
		...
		// else select the edge hovered by the cursor (if any) - works even if
		// no knowledge about logical structure
		closestNode = null;
		Vector pum = mainView.getCursor().getPicker()
				.getIntersectingPaths(mainCamera, 10);
		if (pum.size() > 0) {
	...
	public void startLinkSliding(final double press_vx, final double press_vy,
			int px, int py) {
		mainView.getCursor().setVisibility(false);
		isLinkSliding = true;
		screen_cursor_x = px + panelWidth / 2;
		screen_cursor_y = py + panelHeight / 2;
		mainView.getPanel().setNoEventCoordinates(panelWidth / 2,
				panelHeight / 2);
		try {
			awtRobot = new Robot();
		...
		// add cursor on link
		slideCursor = new SICircle(press_vx, press_vy, 0, SLIDER_CURSOR_SIZE,
				SLIDER_CURSOR_FILL, ConfigManager.HIGHLIGHT_COLOR);
		slideCursor.setStroke(new BasicStroke(2f));
		// slideCursor.setFilled(false);
		mSpace.addGlyph(slideCursor);
		// display selection radius, circular zone that allows for arc switching
		// when starting from a node
		if (closestNode != null) {
			selectionRadius = new SICircle(closestNode.getShape().vx,
					closestNode.getShape().vy, 0, SELECTION_RADIUS,
					Color.WHITE, SELECTION_RADIUS_COLOR);
		...
		Animation a = animator.getAnimationFactory().createCameraTranslation(
				200, mainCamera, new Point2D.Double(press_vx, press_vy), false,
				SlowInSlowOutInterpolator.getInstance(), new EndAction() {
					@Override
	...
	public void linkSlider(double vx, double vy, boolean centerCursor) {
		if (lscs == null) {
			...
			// linkSlider(...) gets fired by the animation at the end of
			// startLinkSliding,
			// e.g., when clicking in LinkSliding mode
			return;
		}
		boolean withinSelectionRadius = mainView.getCursor().getPicker()
				.isPicked(selectionRadius);
		...
		if (!withinSelectionRadius || centerCursor) {
			...
			double shortestDistance = Math.sqrt((tPos.getX() - mtPos.getX())
					* (tPos.getX() - mtPos.getX())
					+ (cPos.getY() - mtPos.getY())
					* (cPos.getY() - mtPos.getY()));
			double distance;
			for (int i = 0; i < lsci; i++) {
				lscs[i].updateMousePosition(mtPos);
				tPos = lscs[i].getPositionAlongPath();
				distance = Math.sqrt((tPos.getX() - mtPos.getX())
						* (tPos.getX() - mtPos.getX())
						+ (cPos.getY() - mtPos.getY())
						* (cPos.getY() - mtPos.getY()));
				if (distance < shortestDistance) {
		...
		// mainCamera.setAltitude((float)(Camera.DEFAULT_FOCAL/lsc.getScale() -
		// Camera.DEFAULT_FOCAL));
	}

	public void endLinkSliding() {
		mainView.getPanel().setNoEventCoordinates(ViewPanel.NO_COORDS,
				ViewPanel.NO_COORDS);
...
class RingManager {
	...
	Ring getRing(double direction, double size, double ringStep) {
		...
		// look for a ring where the new object could be placed, starting with
		// the innermost one
		for (int i = 0; i < rings.length; i++) {
			double a = Math.abs(Math.atan2(size, rings[i].rank * ringStep));
			if (!rings[i].intersectsConeOfInfluence(direction - a, direction
					+ a)) {
...
class Ring {
	...
	// /* cones of influence, for each item, first element is the smallest angle
	// in [0, 2Pi[, second the largest angle in [0, 2Pi[ */
	...
	void addNode(double a1, double a2) {
		...
		// System.arraycopy(broughtPositions, 0, tp, 0,
		// broughtPositions.length);
		// tp[nodes.length] = p;
		// broughtPositions = tp;
...
class DistanceComparator implements java.util.Comparator {
	...
	public int compare(Object o1, Object o2) {
		...
		double d1 = (centerShape.vx - n1.vx) * (centerShape.vx - n1.vx)
				+ (centerShape.vy - n1.vy) * (centerShape.vy - n1.vy);
		double d2 = (centerShape.vx - n2.vx) * (centerShape.vx - n2.vx)
				+ (centerShape.vy - n2.vy) * (centerShape.vy - n2.vy);
		if (d1 < d2) {
			return -1;
		} else if (d1 > d2) {
			return 1;
