...
import Parser (parseAnyAsSwf)
import qualified PrettyCode as PC
import qualified PrettyTree as PT
import Options (commandlineArgs, Options(..))
import qualified Data.ByteString.Lazy as B
import ProgInfo (toTypeInfo, loadTypeInfos, toTypeGr, SymbolTables(..), saveTypeInfo, TypeInfo(..))
import ByteCode
import TrfInjectAbc
import TrfInjectRefl
import TrfInjectSerializable
import GathSymbolTables (symInfoSwf)
import PrettyUtil
import Data.Monoid
import ExtractAbc
import SwfFileCheck (checkSwfFile)
import ParamAnalysis
import Spec
import Stats
import GenInh (generateInheritanceFile)
import GenSymLib
import DeadCodeRemoval
import Consolidate
import ForceAstEval (forceAstEval)
import Boogie (swfToBoogie)
...
main = do
  ...
  when (source /= "") $ do
    ...
    case optExportBoogie opts of
      Nothing   -> return ()
      Just file -> do
        info opts "translating AS bytecode to Boogie"
        swfToBoogie swf file
