...
*
*/
public class SEMsextractor_onlyLastEvent extends SEQsextractor{
	...
	public Vector<Path> run_allSequences_K(boolean reset,String originalFsmPath,String outPathFolder, String fileName, int k,String typeOfVisit){
		...
		selectSEM_onlylastevent_Paths_OfLengthK(fsm); //paths_K_sem
	...
	public Vector<Path> get_allSequences_K(boolean reset,String originalFsmPath,int k,String typeOfVisit){
		...
		selectSEM_onlylastevent_Paths_OfLengthK(fsm);
	...
	protected void selectSEM_onlylastevent_Paths_OfLengthK(FSM fsm){
		...
		for (Path path : paths_K) {
			...
			if ( (path.getEdges().size()<3) ) {
				s0=fsm.getStartNode();
				e1=path.getEdges().get(0);
			}else{
				s0=path.getEdges().get(path.getEdges().size()-3).getTarget();
				e1=path.getEdges().get(path.getEdges().size()-2);
			}
